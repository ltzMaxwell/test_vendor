...
func NewRouter(e *Echoic) *Router {
	return &Router{
		tree: &node{
...
func (r *Router) Add(method, path string, h HandlerFunc, e *Echoic) {
...
func (r *Router) insert(method, path string, h HandlerFunc, t kind, ppath string, pnames []string, e *Echoic) {
	// Adjust max param
	l := len(pnames)
	if *e.maxParam < l {
		*e.maxParam = l
	...
	for {
		...
		if l == 0 {
			...
			// Split node
			n := newNode(cn.kind, cn.prefix[l:], cn, cn.children, cn.methodHandler, cn.ppath, cn.pnames, cn.echoic)
...
func newNode(t kind, pre string, p *node, c children, mh *methodHandler, ppath string, pnames []string, e *Echoic) *node {
	return &node{
		kind:          t,
		label:         pre[0],
		prefix:        pre,
...
func (r *Router) Find(method, path string, ctx *Context) (h HandlerFunc, e *Echoic) {
	h = notFoundHandler
	e = r.echoic
	cn := r.tree // Current node as root
...
func (r *Router) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	c := r.echoic.pool.Get().(*Context)
	h, _ := r.Find(req.Method, req.URL.Path, c)
	c.reset(req, w, r.echoic)
	if err := h(c); err != nil {
		r.echoic.httpErrorHandler(err, c)
	}
	r.echoic.pool.Put(c)
}
