...
func NewContext(req *http.Request, res *Response, e *Echoic) *Context {
	return &Context{
		request:  req,
		response: res,
		echoic:   e,
...
func (c *Context) Bind(i interface{}) error {
	return c.echoic.binder.Bind(c.request, i)
}

// Render renders a template with data and sends a text/html response with status
// code. Templates can be registered using `Echo.SetRenderer()`.
func (c *Context) Render(code int, name string, data interface{}) (err error) {
	if c.echoic.renderer == nil {
		return RendererNotRegistered
	}
	buf := new(bytes.Buffer)
	if err = c.echoic.renderer.Render(buf, name, data); err != nil {
		return
	}
...
func (c *Context) File(path, name string, attachment bool) (err error) {
	...
	if err = c.echoic.serveFile(dir, file, c); err != nil {
...
func (c *Context) Error(err error) {
	c.echoic.httpErrorHandler(err, c)
}

// Echo returns the `Echo` instance.
func (c *Context) Echo() *Echoic {
	return c.echoic
}

func (c *Context) reset(r *http.Request, w http.ResponseWriter, e *Echoic) {
	c.request = r
