...
func (e *Echoic) Router() *Router {
	return e.router
}

// DefaultHTTPErrorHandler invokes the default HTTP error handler.
func (e *Echoic) DefaultHTTPErrorHandler(err error, c *Context) {
	e.defaultHTTPErrorHandler(err, c)
}

// SetHTTPErrorHandler registers a custom Echo.HTTPErrorHandler.
func (e *Echoic) SetHTTPErrorHandler(h HTTPErrorHandler) {
	e.httpErrorHandler = h
}

// SetBinder registers a custom binder. It's invoked by Context.Bind().
func (e *Echoic) SetBinder(b Binder) {
	e.binder = b
}

// SetRenderer registers an HTML template renderer. It's invoked by Context.Render().
func (e *Echoic) SetRenderer(r Renderer) {
	e.renderer = r
}

// SetDebug enable/disable debug mode.
func (e *Echoic) SetDebug(on bool) {
	e.debug = on
}

// Debug returns debug mode (enabled or disabled).
func (e *Echoic) Debug() bool {
	return e.debug
}

// AutoIndex enable/disable automatically creating an index page for the directory.
func (e *Echoic) AutoIndex(on bool) {
	e.autoIndex = on
}
...
func (e *Echoic) Hook(h http.HandlerFunc) {
	e.hook = h
}

// Use adds handler to the middleware chain.
func (e *Echoic) Use(m ...Middleware) {
	for _, h := range m {
...
func (e *Echoic) Connect(path string, h Handler) {
	e.add(CONNECT, path, h)
}

// Delete adds a DELETE route > handler to the router.
func (e *Echoic) Delete(path string, h Handler) {
	e.add(DELETE, path, h)
}

// Get adds a GET route > handler to the router.
func (e *Echoic) Get(path string, h Handler) {
	e.add(GET, path, h)
}

// Head adds a HEAD route > handler to the router.
func (e *Echoic) Head(path string, h Handler) {
	e.add(HEAD, path, h)
}

// Options adds an OPTIONS route > handler to the router.
func (e *Echoic) Options(path string, h Handler) {
	e.add(OPTIONS, path, h)
}

// Patch adds a PATCH route > handler to the router.
func (e *Echoic) Patch(path string, h Handler) {
	e.add(PATCH, path, h)
}

// Post adds a POST route > handler to the router.
func (e *Echoic) Post(path string, h Handler) {
	e.add(POST, path, h)
}

// Put adds a PUT route > handler to the router.
func (e *Echoic) Put(path string, h Handler) {
	e.add(PUT, path, h)
}

// Trace adds a TRACE route > handler to the router.
func (e *Echoic) Trace(path string, h Handler) {
	e.add(TRACE, path, h)
}

// Any adds a route > handler to the router for all HTTP methods.
func (e *Echoic) Any(path string, h Handler) {
...
func (e *Echoic) Match(methods []string, path string, h Handler) {
	for _, m := range methods {
		e.add(m, path, h)
	}
}

func (e *Echoic) add(method, path string, h Handler) {
	path = e.prefix + path
...
func (e *Echoic) ServeFile(path, file string) {
	e.Get(path, func(c *Context) error {
...
func (e *Echoic) serveFile(dir, file string, c *Context) (err error) {
	fs := http.Dir(dir)
	f, err := fs.Open(file)
	if err != nil {
		return NewHTTPError(http.StatusNotFound)
...
func (e *Echoic) Group(prefix string, m ...Middleware) *Group {
	g := &Group{*e}
	g.echoic.prefix += prefix
	if len(m) == 0 {
		mw := make([]MiddlewareFunc, len(g.echoic.middleware))
		copy(mw, g.echoic.middleware)
		g.echoic.middleware = mw
	} else {
		g.echoic.middleware = nil
		g.Use(m...)
	}
	return g
}
...
func (e *Echoic) URI(h Handler, params ...interface{}) string {
...
func (e *Echoic) URL(h Handler, params ...interface{}) string {
	return e.URI(h, params...)
}

// Routes returns the registered routes.
func (e *Echoic) Routes() []Route {
	return e.router.routes
}

// ServeHTTP implements `http.Handler` interface, which serves HTTP requests.
func (e *Echoic) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if e.hook != nil {
		e.hook(w, r)
	}
...
func (e *Echoic) Server(addr string) *http.Server {
	s := &http.Server{Addr: addr, Handler: e}
	return s
}

// Run runs a server.
func (e *Echoic) Run(addr string) {
	e.run(e.Server(addr))
}

// RunTLS runs a server with TLS configuration.
func (e *Echoic) RunTLS(addr, crtFile, keyFile string) {
	e.run(e.Server(addr), crtFile, keyFile)
}

// RunServer runs a custom server.
func (e *Echoic) RunServer(s *http.Server) {
	e.run(s)
}

// RunTLSServer runs a custom server with TLS configuration.
func (e *Echoic) RunTLSServer(s *http.Server, crtFile, keyFile string) {
	e.run(s, crtFile, keyFile)
}

func (e *Echoic) run(s *http.Server, files ...string) {
	s.Handler = e
}
